...
{
  __attribute__((cleanup(g_autoptr_cleanup_generic_gfree))) char *escaped = ((void*)0);
  __attribute__((cleanup(g_autoptr_cleanup_generic_gfree))) gchar *pretty = ((void*)0);
  int i;
  ...
  pretty = g_strdup_inline (g_strchomp (g_strchug (escaped)));

  for (i = 0; i < (sizeof (rs) / sizeof ((rs)[0])); i++)
    {
      __attribute__((cleanup(glib_autoptr_cleanup_GError))) GError_autoptr error = ((void*)0);
      __attribute__((cleanup(glib_autoptr_cleanup_GRegex))) GRegex_autoptr re = ((void*)0);
      __attribute__((cleanup(g_autoptr_cleanup_generic_gfree))) gchar *new = ((void*)0);

      re = g_regex_new (rs[i].regex, 0, 0, &error);
      if (re == ((void*)0))
        {
          g_log ("system-info-properties", G_LOG_LEVEL_WARNING, "Error building regex: %s", error->message);
          continue;
        }
      ...
      if (error != ((void*)0))
        {
          g_log ("system-info-properties", G_LOG_LEVEL_WARNING, "Error replacing %s: %s", rs[i].regex, error->message);
          continue;
        }

      g_free (pretty);
      pretty = ((__typeof__ (*&new)) (g_steal_pointer) (&new));
    }

  return ((__typeof__ (*&pretty)) (g_steal_pointer) (&pretty));
}
...
remove_duplicate_whitespace (const char *old)
{
  ...
  if (new == ((void*)0))
    {
      g_log ("system-info-properties", G_LOG_LEVEL_WARNING, "Error replacing string: %s", error->message);
      return g_strdup_inline (old);
    }

  return ((__typeof__ (*&new)) (g_steal_pointer) (&new));
}

char *
info_cleanup (const char *input)
{
  __attribute__((cleanup(g_autoptr_cleanup_generic_gfree))) char *pretty = ((void*)0);
