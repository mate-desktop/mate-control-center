...
e_image_chooser_init (EImageChooser *chooser)
{
    ...
    gtk_drag_dest_set (priv->image, 0, image_drag_types,
                       num_image_drag_types, GDK_ACTION_COPY);
                     ...
                     char *data, int length)
{
    ...
    if (pixbuf) {
        ...
        g_signal_emit (chooser, image_chooser_signals [CHANGED], 0);
    ...
    if (!strcmp (target_type, URI_LIST_TYPE)) {
        ...
        if (istream != NULL) {
            ...
            if (info != NULL) {
                ...
                if (success && set_image_from_data (chooser, buf, size))
                    handled = TRUE;
                else
                    g_free (buf);
            }
