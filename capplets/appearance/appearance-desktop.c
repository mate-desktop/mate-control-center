...
static void wp_update_preview (GtkFileChooser*  chooser,
                               AppearanceData*  data);
...
wp_set_sensitivities (AppearanceData *data)
{
    ...
    sensitive = !g_settings_is_writable (data->wp_settings, WP_OPTIONS_KEY)
                || (filename && !strcmp (filename, "(none)"));
    gtk_widget_set_sensitive (data->wp_style_menu, sensitive);

    sensitive = !g_settings_is_writable (data->wp_settings, WP_SHADING_KEY);
    gtk_widget_set_sensitive (data->wp_color_menu, sensitive);

    sensitive = !g_settings_is_writable (data->wp_settings, WP_PCOLOR_KEY);
    gtk_widget_set_sensitive (data->wp_pcpicker, sensitive);

    sensitive = !g_settings_is_writable (data->wp_settings, WP_SCOLOR_KEY);
    gtk_widget_set_sensitive (data->wp_scpicker, sensitive);

    sensitive = !filename || !strcmp (filename, "(none)");
    gtk_widget_set_sensitive (data->wp_rem_button, sensitive);
}
              ...
              const gchar       **uris)
{
    ...
    g_object_set (cr, "mode", GTK_CELL_RENDERER_MODE_ACTIVATABLE,
                  "pixbuf", buttons[0], NULL);
    ...
    g_signal_connect (data->wp_view,
                      "selection-changed",
                      (GCallback) wp_selected_changed_cb,
                      data);
    g_signal_connect (data->wp_view,
                      "button-press-event",
                      G_CALLBACK (wp_button_press_cb),
                      data);
    ...
    g_signal_connect (data->wp_style_menu,
                      "changed",
                      (GCallback) wp_scale_type_changed,
                      data);

    data->wp_color_menu = appearance_capplet_get_widget (data, "wp_color_menu");

    g_signal_connect (data->wp_color_menu,
                      "changed",
                      (GCallback) wp_shade_type_changed,
                      data);

    data->wp_scpicker = appearance_capplet_get_widget (data, "wp_scpicker");

    g_signal_connect (data->wp_scpicker,
                      "color-set",
                      (GCallback) wp_scolor_changed,
                      data);

    data->wp_pcpicker = appearance_capplet_get_widget (data, "wp_pcpicker");

    g_signal_connect (data->wp_pcpicker,
                      "color-set",
                      (GCallback) wp_scolor_changed,
                      data);
    ...
    g_signal_connect (data->wp_view,
                      "selection-changed",
                      (GCallback) wp_props_wp_selected,
                      data);
    g_signal_connect (data->wp_view,
                      "query-tooltip",
                      (GCallback) wp_view_tooltip_cb,
                      data);
