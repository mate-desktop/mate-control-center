...
static GdkPixbuf*
theme_get_thumbnail_from_cache (MateThemeMetaInfo*   info,
                                AppearanceData*      data)
{
...
static void
theme_thumbnail_done_cb (GdkPixbuf      *pixbuf,
                         gchar          *theme_name,
                         AppearanceData *data)
{
...
static void
theme_thumbnail_generate (MateThemeMetaInfo*    info,
                          AppearanceData*       data)
{
...
static gchar *
theme_get_selected_name (GtkIconView    *icon_view,
                         AppearanceData *data)
{
    gchar *name = NULL;
...
static void
theme_select_iter (GtkIconView  *icon_view,
                   GtkTreeIter  *iter)
{
    GtkTreePath *path;
...
static void
theme_select_name (GtkIconView  *icon_view,
                   const gchar  *theme)
{
    GtkTreeIter iter;
    GtkTreeModel *model = gtk_icon_view_get_model (icon_view);
...
static void
theme_set_custom_from_theme (const MateThemeMetaInfo    *info,
                             AppearanceData             *data)
{
...
theme_message_area_update (AppearanceData *data)
{
    ...
    if (!show_error)
    {
        if (theme->background_image != NULL)
        {
            ...
            show_apply_background = (!background || strcmp (theme->background_image, background) != 0);
            g_free (background);
        }
    ...
    if (data->theme_message_area == NULL)
    {
        ...
        if (!show_apply_background && !show_revert_font &&
            !show_apply_font && !show_error)
        {
            return;
        }
...
static void
theme_selection_changed_cb (GtkWidget       *icon_view,
                            AppearanceData  *data)
{
    GList *selection;
    MateThemeMetaInfo *theme = NULL;
...
static void
theme_custom_cb (GtkWidget      *button,
                 AppearanceData *data)
{
    GtkWidget *w, *parent;
...
static void
theme_save_cb (GtkWidget        *button,
               AppearanceData   *data)
{
...
static void
theme_install_cb (GtkWidget         *button,
                  AppearanceData    *data)
{
    mate_theme_installer_run (GTK_WINDOW (appearance_capplet_get_widget (data, "appearance_window")));
}

static void
theme_delete_cb (GtkWidget      *button,
                 AppearanceData *data)
{
    GtkIconView *icon_view = GTK_ICON_VIEW (appearance_capplet_get_widget (data, "theme_list"));
    GList *selected = gtk_icon_view_get_selected_items (icon_view);
                             ...
                             AppearanceData     *data)
{
    ...
    if (uris != NULL && uris[0] != NULL) {
        ...
        mate_theme_install (f, GTK_WINDOW (appearance_capplet_get_widget (data, "appearance_window")));
        g_object_unref (f);
    }
...
static void
background_or_font_changed (GSettings       *settings,
                            gchar           *key,
                            AppearanceData* data)
{
    theme_message_area_update(data);
...
themes_init (AppearanceData* data)
{
    ...
    data->theme_store = theme_store = gtk_list_store_new (NUM_COLS,
                                                          GDK_TYPE_PIXBUF,
                                                          G_TYPE_STRING,
                                                          G_TYPE_STRING);
