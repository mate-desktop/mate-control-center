...
const gchar *
wp_item_option_to_string (MateBGPlacement type)
{
    const gchar *retval = "";
    switch (type)
    {
...
const gchar *
wp_item_shading_to_string (MateBGColorType type)
{
    const gchar *retval = "";
    switch (type)
    {
        ...
        case MATE_BG_COLOR_H_GRADIENT:
            retval = "horizontal-gradient";
            break;
        case MATE_BG_COLOR_V_GRADIENT:
            retval = "vertical-gradient";
            break;
    }
    return retval;
}
...
static void
set_bg_properties (MateWPItem *item)
{
    if (item->filename)
    {
...
void
mate_wp_item_ensure_mate_bg (MateWPItem *item)
{
    if (!item->bg)
    {
        item->bg = mate_bg_new ();
...
void
mate_wp_item_update (MateWPItem *item)
{
    GSettings *settings;
...
MateWPItem *
mate_wp_item_new (const gchar *                 filename,
                  ...
                  MateDesktopThumbnailFactory * thumbnails)
{
        ...
        || strcmp (item->fileinfo->mime_type, "application/xml") == 0))
    {
        ...
        if (g_utf8_validate (item->fileinfo->name, -1, NULL))
        {
            ...
            item->name = g_filename_to_utf8 (item->fileinfo->name,
                                             -1, NULL, NULL, NULL);
        }
...
void
mate_wp_item_free (MateWPItem * item)
{
    if (item == NULL)
    {
        return;
...
GdkPixbuf *
mate_wp_item_get_frame_thumbnail (MateWPItem *                  item,
...
GdkPixbuf *
mate_wp_item_get_thumbnail (MateWPItem * item,
                            MateDesktopThumbnailFactory * thumbs,
                            gint width,
                            gint height)
{
