...
mate_wp_xml_get_bool (const xmlNode* parent, const char* prop_name)
{
    ...
    if (parent != NULL && prop_name != NULL)
    {
        ...
        if (prop != NULL)
        {
            ret_val = !g_ascii_strcasecmp((char*) prop, "true") || !g_ascii_strcasecmp((char*) prop, "1");
...
static void
mate_wp_xml_set_bool (const xmlNode*    parent,
...
static void
mate_wp_xml_load_xml (AppearanceData*   data,
                      const char*       filename)
{
    xmlDoc* wplist;
    xmlNode* root;
...
static void
mate_wp_file_changed (GFileMonitor*     monitor,
                      GFile*            file,
                      GFile*            other_file,
...
static void
mate_wp_xml_add_monitor (GFile*             directory,
                         AppearanceData*    data)
{
    GError* error = NULL;
...
static void
mate_wp_xml_load_from_dir (const char*      path,
                           AppearanceData*  data)
{
    GFile* directory;
...
static void
mate_wp_list_flatten (const char*   key,
                      MateWPItem*   item,
                      GSList**      list)
{
