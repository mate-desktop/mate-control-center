...
create_meta_theme_pixbuf (ThemeThumbnailData *theme_thumbnail_data)
{
    ...
    g_object_set (gtk_settings_get_default (),
                 "gtk-theme-name",
                 (char *) theme_thumbnail_data->control_theme_name->data,
                 "gtk-font-name",
                 (char *) theme_thumbnail_data->application_font->data,
                 "gtk-icon-theme-name",
                 (char *) theme_thumbnail_data->icon_theme_name->data,
                 "gtk-color-scheme",
                 (char *) theme_thumbnail_data->gtk_color_scheme->data,
                 NULL);
...
create_marco_theme_pixbuf (ThemeThumbnailData *theme_thumbnail_data)
{
    ...
    region = meta_preview_get_clip_region (META_PREVIEW (preview),
                                           MARCO_THUMBNAIL_WIDTH * 1.2,
                                           MARCO_THUMBNAIL_HEIGHT * 1.2);
    pixbuf_apply_mask_region (pixbuf, region);
    cairo_region_destroy (region);
