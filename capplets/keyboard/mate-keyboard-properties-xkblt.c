            ...
            for (i = 0; initial_config.layouts_variants[i] != NULL; i++)
            {
                retval =
                    g_slist_prepend (retval,
                                     g_strdup (initial_config.layouts_variants[i]));
            }
        }
...
xkb_layouts_enable_disable_buttons (GtkBuilder * dialog)
{
    ...
    const int n_selected_layouts =
        gtk_tree_model_iter_n_children (selected_layouts_model, NULL);
...
static void
xkb_layouts_dnd_data_received (GtkWidget * widget,
                               GdkDragContext * dc,
                               gint x, gint y,
...
move_up_selected_layout (GtkWidget * button, GtkBuilder * dialog)
{
    ...
    if (idx != -1)
    {
        ...
        layouts_list = g_slist_insert (layouts_list, node2Remove->data,
                                       idx - 1);
        g_slist_free_1 (node2Remove);
